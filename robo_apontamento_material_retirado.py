...
class Apontamento:
    ...
    def botao_material_retirado(self):
        """ Clicar no botão de materia retirado """
        try:
            # Clicar no botão de materia
            self.wait.until(EC.element_to_be_clickable ((By.ID, "aba4"))).click()
            self.log("Botão de material retirado clicado.")
        except TimeoutException:
            self.log("Botão de material retirado não encontrado.")
    ...
    def preencher_cabecalho(self, row):
        try:
            ...
            # salvar
            apontamento.salvar()
            # Botão material retirado
            apontamento.botao_material_retirado()
            # Acessar iframe secundário
            apontamento._acessar_iframes_secundarios()

    def preencher_material_retirado(self, row):
        try:
            ...
            # Pausa breve                
            sleep(0.2)
            self._interagir_dropdown("controle_chosen", str(row["controle_chosen"]))
            # Pausa breve                
            sleep(0.2)
        ...
        try:
            ...
            try:
                for index, row in df.iterrows():
                    ...
                    try:
                        self.preencher_material_retirado(row)
...
if __name__ == "__main__":
    ...
    try:
        ...
        apontamento.executar_planilha("dados_apontamento_material_retirado.xlsx")
    except Exception as e:
